...
module.exports.outputUniqueCurrencies = function () {
  let items = {}
  iso.forEach((i) => {
    // Make sure they have a name, units and a corrisponding symbol
    if (i.Ccy && i.CcyMnrUnts != 'N.A.' && symbols[i.Ccy]) {
      // If not in object lets add it
      if (!items[i.Ccy]) {
        let fraction = parseInt(i.CcyMnrUnts)
        items[i.Ccy] = {
          count: 0,
          symbol: symbols[i.Ccy] || i.Ccy,
          fraction: fraction
        }
      }
  ...
  // Remove those that have less than a certain amount of usage
  // let usageLimit = 2 // At least 3 need to use it
  // for (const item in items) {
  //   if (items[item].count < usageLimit) {
  //     delete items[item]
  //   }
  // }
